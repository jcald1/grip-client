...
const DEFAULT_SIMULATION_RUN_COLUMNS = (
  <div style={{ display: 'flex' }}>
    <span style={{ marginLeft: '0px', width: '6%' }}>
      Simulation
      <br /> Run ID
    </span>
    <span style={{ marginLeft: '0px', width: '10%' }}>Run Date/Time</span>
    <span style={{ marginLeft: '0px', width: '20%' }}>Filename</span>
    <span style={{ marginLeft: '0px', width: '10%' }}>Status</span>
    <span style={{ marginLeft: '0px', width: '30%' }}>Status/Error Details</span>
  </div>
);
  ...
  let runsToRender = simulationRuns.map(run => {
    ...
    const line = (
      <div
        ...
        key={run.id}
      >
        <span style={{ marginLeft: '0px', width: '6%' }}>{run.id}</span>
        <span style={{ marginLeft: '0px', width: '10%' }}>{runDate}</span>
        <span style={{ marginLeft: '0px', width: '20%' }}>{run.simulation_filename}</span>
        <span style={{ marginLeft: '0px', width: '10%' }}>
          {DEFAULT_SIMULATION_RUN_STATUSES[run.status]}
        </span>
        <span style={{ marginLeft: '0px', width: '30%' }}>{run.status_details}</span>
